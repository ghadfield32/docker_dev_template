...
if docker compose -p docker_dev_template ps -q >/dev/null 2>&1; then
    docker compose -p docker_dev_template down
...
docker compose -p docker_dev_template build --no-cache datascience
status_ok "DevContainer rebuilt successfully"

# Step 4: Start the container
log "Step 4: Starting DevContainer..."
docker compose -p docker_dev_template up -d
...
if docker compose -p docker_dev_template ps | grep -q "Up"; then
    status_ok "Container is running"
else
    status_error "Container failed to start"
    echo "Check logs with: docker compose -p docker_dev_template logs -f"
    exit 1
fi

# Step 7: Run setup script inside container
log "Step 7: Running setup script inside container..."
CONTAINER_NAME=$(docker compose -p docker_dev_template ps -q datascience)
...
echo "  View logs: docker compose -p docker_dev_template logs -f"
echo "  Enter container: docker exec -it $CONTAINER_NAME bash"
echo "  Stop container: docker compose -p docker_dev_template down"
